...
class Conv_Layer:
    ...
    def __init__(
        ...
        loc_ts_size = (
            win_l * win_l * n_pol
        )  # OpenCL local size for kernels operating
        ...
        centroids = np.zeros(
            [batch_size, n_clusters, win_l, win_l, n_pol], dtype=w_prec
        )
        centroids[:] = np.random.rand(
            n_clusters, win_l, win_l, n_pol
        )  # *0.001
        ...
        centroids_update = np.zeros(
            [batch_size, n_clusters, win_l, win_l, n_pol], dtype=w_prec
        )

        # Zeropadding for conv
        time_context = np.zeros(
            [batch_size, res_x + win_l - 1, res_y + win_l - 1, n_pol],
            dtype=np.int32,
        )  # time context matrix
        time_context_mask = np.zeros(
            [batch_size, res_x + win_l - 1, res_y + win_l - 1, n_pol],
            dtype=np.int32,
        )
        ...
        partial_sum = np.zeros(
            [batch_size, n_clusters, loc_ts_size], dtype=dist_prec
        )

        time_surface = np.zeros(
            [batch_size, win_l, win_l, n_pol], dtype=ts_prec
        )  # TODO add zeropadding

        if fb_signal:
            output_S = np.zeros([batch_size, res_x, res_y], dtype=np.float32)
            output_dS = np.zeros([batch_size, res_x, res_y], dtype=np.float32)
            fb_time_context = np.zeros(
                [batch_size, n_clusters], dtype=np.int32
            )
            fb_time_context_mask = np.zeros(
                [batch_size, n_clusters], dtype=np.int32
            )
            fb_partial_sum = np.zeros(
                [batch_size, n_clusters], dtype=dist_prec
            )
    ...
    def buffer_init(self):
        ...
        n_clusters_bf = cl.Buffer(
            ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=np.int32(n_clusters)
        )
        tau_bf = cl.Buffer(
            ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=np.int32(tau)
        )
        res_x_bf = cl.Buffer(
            ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=np.int32(res_x)
        )
        res_y_bf = cl.Buffer(
            ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=np.int32(res_y)
        )
        win_l_bf = cl.Buffer(
            ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=np.int32(win_l)
        )
        n_pol_bf = cl.Buffer(
            ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=np.int32(n_pol)
        )
        lrate_bf = cl.Buffer(
            ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=np.float32(lrate)
        )
        s_gain_bf = cl.Buffer(
            ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=np.float32(s_gain)
        )
        th_lrate_bf = cl.Buffer(
            ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=np.float32(th_lrate)
        )
        th_decay_bf = cl.Buffer(
            ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=np.float32(th_decay)
        )
        win_lkt_bf = cl.Buffer(
            ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=win_lkt
        )

        self.buffers = {
            "n_clusters_bf": n_clusters_bf,
            "tau_bf": tau_bf,
            "res_x_bf": res_x_bf,
            "res_y_bf": res_y_bf,
            "win_l_bf": win_l_bf,
        ...
        centroids_bf = cl.Buffer(
            ctx, mf.READ_WRITE | mf.COPY_HOST_PTR, hostbuf=centroids
        )
        dcentroids_bf = cl.Buffer(
            ctx, mf.READ_WRITE | mf.COPY_HOST_PTR, hostbuf=dcentroids
        )
        centroids_update_bf = cl.Buffer(
            ctx, mf.READ_WRITE | mf.COPY_HOST_PTR, hostbuf=centroids_update
        )
        time_context_bf = cl.Buffer(
            ctx, mf.READ_WRITE | mf.COPY_HOST_PTR, hostbuf=time_context
        )
        time_context_mask_bf = cl.Buffer(
            ctx, mf.READ_WRITE | mf.COPY_HOST_PTR, hostbuf=time_context_mask
        )
        thresholds_bf = cl.Buffer(
            ctx, mf.READ_WRITE | mf.COPY_HOST_PTR, hostbuf=thresholds
        )
        thresholds_update_bf = cl.Buffer(
            ctx, mf.READ_WRITE | mf.COPY_HOST_PTR, hostbuf=thresholds_update
        )
        closest_c_bf = cl.Buffer(
            ctx, mf.READ_WRITE | mf.COPY_HOST_PTR, hostbuf=closest_c
        )
        distances_bf = cl.Buffer(
            ctx, mf.READ_WRITE | mf.COPY_HOST_PTR, hostbuf=distances
        )
        partial_sum_bf = cl.Buffer(
            ctx, mf.READ_WRITE | mf.COPY_HOST_PTR, hostbuf=partial_sum
        )
        time_surface_bf = cl.Buffer(
            ctx, mf.READ_WRITE | mf.COPY_HOST_PTR, hostbuf=time_surface
        )
        if self.fb_signal:
            output_S_bf = cl.Buffer(
                ctx, mf.READ_WRITE | mf.COPY_HOST_PTR, hostbuf=output_S
            )
            output_dS_bf = cl.Buffer(
                ctx, mf.READ_WRITE | mf.COPY_HOST_PTR, hostbuf=output_dS
            )
            fb_time_context_bf = cl.Buffer(
                ctx, mf.READ_WRITE | mf.COPY_HOST_PTR, hostbuf=fb_time_context
            )
            fb_time_context_mask_bf = cl.Buffer(
                ctx,
                mf.READ_WRITE | mf.COPY_HOST_PTR,
                hostbuf=fb_time_context_mask,
            )
            fb_partial_sum_bf = cl.Buffer(
                ctx, mf.READ_WRITE | mf.COPY_HOST_PTR, hostbuf=fb_partial_sum
            )
            fb_tau_bf = cl.Buffer(
                ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=np.int32(fb_tau)
            )
        else:
            output_S_bf = None
            output_dS_bf = None
    ...
    def batch_flush(self, queue):
        ...
        centroids_update = np.zeros(
            [batch_size, n_clusters, win_l, win_l, n_pol], dtype=self.__w_prec
        )
        thresholds_update = np.zeros(
            [batch_size, n_clusters], dtype=self.__w_prec
        )
        ...
        if self.fb_signal:
            ...
            cl.enqueue_copy(
                queue, fb_time_context_mask_bf, fb_time_context_mask
            ).wait()
            self.buffers["output_S_bf"] = output_S_bf
            self.buffers["output_dS_bf"] = output_dS_bf
            self.buffers["fb_time_context_bf"] = fb_time_context_bf
    ...
    def centroid_buffer_copy(self, queue):
        ...
        """
        cl.enqueue_copy(
            queue, self.variables["centroids"], self.buffers["centroids_bf"]
        )

        cl.enqueue_copy(
            queue, self.variables["thresholds"], self.buffers["thresholds_bf"]
        ).wait()
    ...
    def program_build(self):
        ...
        programtxt = (
            tcont_update + ts_gen + partial_dist + reduce_dist + infer_end
        )
        ...
        self.program = cl.Program(ctx, programtxt).build(
    ...
    def init_infer(self, ext_buffer, queue, time_surface_bf):
        ...
        """
        self.queue_init_time_surface_generation(
            ext_buffer, queue, time_surface_bf
        )
        self.queue_context_update(ext_buffer, queue)
    ...
    def queue_init_time_surface_generation(
        self, ext_buffer, queue, time_surface_bf
    ):
        """
        Method to queue the ts_gen kernel
    ...
    def queue_feedback_context_update(self, ext_buffer, queue):
        ...
        self.program.fb_context_update(
            queue,
            global_space,
            local_space,
            ts_bf,
