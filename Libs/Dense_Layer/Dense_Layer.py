...
class Dense_Layer:
    ...
    def __init__(
        ...
        centroids = np.zeros(
            [batch_size, n_clusters, res_x, res_y, n_pol], dtype=w_prec
        )
        ...
        time_context = np.zeros(
            [batch_size, res_x, res_y, n_pol], dtype=np.int32
        )  # time context matrix
        ...
        time_surface = np.zeros(
            [batch_size, res_x, res_y, n_pol], dtype=ts_prec
        )
    ...
    def program_build(self):
        ...
        dr = "Libs/Dense_Layer/feedback_cl/"
        ...
        f = open(dr + "th_reduce.cl", "r")
        th_reduce = "".join(f.readlines())
        f = open(dr + "w_reduce.cl", "r")
        w_reduce = "".join(f.readlines())

        programtxt = programtxt + th_reduce + w_reduce
        ...
        programtxt = (
            programtxt + init_infer_end + init_w_update + init_th_update
        )
    ...
    def queue_experimental_thresholds_reduce(self, ext_buffer, queue):
        """
        Method to queue the th_reduce kernel
