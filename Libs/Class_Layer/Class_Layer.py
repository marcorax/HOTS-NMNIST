...
class Class_Layer:
    ...
    def buffer_init(self):
        ...
        if self.fb_signal:
            ...
            correct_response_bf = cl.Buffer(
                ctx, mf.READ_WRITE | mf.COPY_HOST_PTR, hostbuf=correct_response
            )
        else:
            output_S_bf = None
            output_dS_bf = None
    ...
    def batch_flush(self, queue):
        ...
        correct_ev = np.zeros([batch_size], dtype=np.int32)
        centroids_update = np.zeros(
            [batch_size, n_clusters, res_x, res_y, n_pol], dtype=self.__w_prec
        )
        ...
        if self.fb_signal:
            ...
            cl.enqueue_copy(
                queue, correct_response_bf, correct_response
            ).wait()
            self.buffers["output_S_bf"] = output_S_bf
            self.buffers["output_dS_bf"] = output_dS_bf
            self.buffers["fb_time_context_bf"] = fb_time_context_bf
